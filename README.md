# Short Video Story Pipeline

一个面向 Windows 的本地公开视频素材研究工具：将你选择的公开视频下载到本地，转写语音，并生成结构化故事卡片，供 AI 或人工进一步核实、分析和原创改编。

它最初从抖音素材研究开始，现在定位为多平台公开短视频工作流。底层下载能力由 `yt-dlp` 提供，支持范围取决于 `yt-dlp` 当前提取器和平台公开访问规则。

它不是全站爬虫，也不用于绕过登录、验证码、付费墙、隐私设置或访问控制。

## 支持平台

当前版本会把 `urls.txt` 里的链接交给 `yt-dlp` 处理。经本地提取器检查，常见可尝试平台包括：
...
注意：平台规则和 `yt-dlp` 支持状态会变化。公开视频通常最稳定；需要登录、验证码、私密权限或特殊地区限制的内容不属于本工具的一键处理范围。
...
2. 选择 `Download ZIP`。
3. 解压下载的 ZIP。
4. 如果下载的是 Release 版本，优先双击 `ShortVideoStoryPipeline.exe`。
5. 如果没有 exe，或者 exe 被安全软件拦截，可以双击备用入口 `START.cmd`。

图形界面里按顺序操作：

1. 首次点击 `安装或修复工具`。
2. 点击 `编辑视频链接`，每行粘贴一个公开视频链接并保存。
3. 点击 `开始处理素材`。
...
1. 双击 `START.cmd`。
2. 首次选择 `1. 首次安装或修复工具`。
3. 安装完成后选择 `2. 编辑视频链接`，每行粘贴一个公开视频链接并保存。
4. 选择 `3. 开始下载、转写并生成故事卡`。
...
项目的 exe 是一个 Windows 图形启动器，核心处理逻辑仍然放在 PowerShell 脚本里。这样做的好处是：

- 普通用户双击 exe 使用。
- 开发者仍然可以直接阅读和修改脚本。
- 下载的视频、模型和用户链接会保留在项目文件夹中，不会因为 exe 每次自解压而丢失。
